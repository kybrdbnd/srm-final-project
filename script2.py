...
def drawChartSMA(close,date,sma):
    ...
    if(sma[len(sma)-1]<close[len(close)-1]):
        ...
        print("Support is Rs." , min)
        print("Resistance is Rs.", max)
    if((sma[len(sma)-2]>=close[len(close)-2]) and (sma[len(sma)-1]<close[len(close)-1])):
        print("SMA:Buy Signal Is Generated.")
    elif((sma[len(sma)-2]<=close[len(close)-2]) and (sma[len(sma)-1]>close[len(close)-1])):
        print("SMA:Sell Signal is generated.")
    else:
        print("SMA:No Buy/Sell Signal Generated.")
...
def bollinger_band(close,window):
    ...
    q = cl[len(cl)-1]-lo[len(lo)-1]
    p = up[len(up)-1]-cl[len(cl)-1]
    if(p>q and (cl[len(cl)-1]>lo[len(lo)-1])):
         print("Bollinger Band: Buy Signal Generated")
    elif(p<q and (cl[len(cl)-1]<up[len(up)-1])):
         print("Bollinger Band: Sell Signal Generated")
...
def plotMACD(close,ema9,emaslow,emafast,macd,date):
    ...
    ema9=ema9.tolist()
    macd = macd.tolist()
    if((ema9[len(ema9)-1]<macd[len(macd)-1]) and (ema9[len(ema9)-2]>macd[len(macd)-2])):
       print("MACD:Buy Signal Generated")
    elif((ema9[len(ema9)-1]>macd[len(macd)-1]) and (ema9[len(ema9)-2]<macd[len(macd)-2])):
       print("MACD:Sell Signal Generated")
    else:
       print("MACD: No Buy/Sell Signal Generated")
    plt.show() 
...
try:
    ...
    try:
        ...
        n=20
        k=close.tolist()
        print("Current Closing Price",k[len(k)-1])
        ...
        l = close.tolist() 
        if((ema20[len(ema20)-2]<ema50[len(ema50)-2] ) and (ema20[len(ema20)-1]>ema50[len(ema50)-1])):
            print("EMA:Buy Signal Generated.")
        elif((ema20[len(ema20)-2]>ema50[len(ema50)-2]) and (ema20[len(ema20)-1]<ema50[len(ema50)-1])):
            print("EMA:Sell Signal Generated")
        else:
            print("EMA:No Buy/Sell Signal Generated")
        ...
        f=predict_price(dates,close,30)
        f=float("{0:.2f}".format(f))
        print("Next Predicted Price:",f)
