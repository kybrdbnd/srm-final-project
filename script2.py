...
def drawChartSMA(close, date, sma):
    ...
    if sma[len(sma) - 1] < close[len(close) - 1]:
        ...
        print('Support is Rs.', min)
        print('Resistance is Rs.', max)
    if sma[len(sma) - 2] >= close[len(close) - 2] and sma[len(sma) - 1] \
            < close[len(close) - 1]:
        print('SMA:Buy Signal Is Generated.')
    elif sma[len(sma) - 2] <= close[len(close) - 2] and sma[len(sma)
                                                            - 1] > close[len(close) - 1]:
...
def plotMACD(
    ...
    ema9 = ema9.tolist()
    macd = macd.tolist()
    if ema9[len(ema9) - 1] < macd[len(macd) - 1] and ema9[len(ema9)
                                                          - 2] > macd[len(macd) - 2]:
        print('MACD:Buy Signal Generated')
    elif ema9[len(ema9) - 1] > macd[len(macd) - 1] and ema9[len(ema9)
                                                            - 2] < macd[len(macd) - 2]:
...
try:
    ...
    try:
        ...
        l = close.tolist()
        if ema20[len(ema20) - 2] < ema50[len(ema50) - 2] \
                and ema20[len(ema20) - 1] > ema50[len(ema50) - 1]:
            print('EMA:Buy Signal Generated.')
        elif ema20[len(ema20) - 2] > ema50[len(ema50) - 2] \
                and ema20[len(ema20) - 1] < ema50[len(ema50) - 1]:
